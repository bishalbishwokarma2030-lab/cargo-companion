      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button onClick={openCreate} className="bg-gradient-primary"><Plus className="mr-1 h-4 w-4" />Create Shipment</Button>
          </>
        }
      />
      <div className="p-6">
        ...
        <DataTable<Shipment>
          ...
          columns={[
            ...
            { key: "container", header: "Container", render: (r) => <div><div className="text-xs"><b>Name:</b> {r.container_name}</div><div className="text-xs"><b>Type:</b> <span className="text-primary">{r.container_type}</span></div></div> },
            { key: "driver", header: "Driver Details", render: (r) => <div><div className="text-xs"><b>Name:</b> {r.driver_name || "—"}</div><div className="text-xs"><b>Phone:</b> {r.driver_phone || "—"}</div></div> },
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-lg border border-border bg-card p-4">
              <div className="mb-3 text-sm font-semibold text-primary">Container Details</div>
              <div className="space-y-3">
                ...
                <F label="Container Type *">
                  <Select value={form.container_type} onValueChange={(v) => setForm({ ...form, container_type: v })}>
                    ...
                    <SelectContent><SelectItem value="Truck">Truck</SelectItem><SelectItem value="Container">Container</SelectItem><SelectItem value="Van">Van</SelectItem></SelectContent>
                  </Select>
                </F>
              </div>
            </div>
            <div className="space-y-4">
              <div className="rounded-lg border border-border bg-card p-4">
                <div className="mb-3 text-sm font-semibold text-primary">Lot Details</div>
                <F label="Lot No. *"><Input value={form.lot_no} onChange={(e) => setForm({ ...form, lot_no: e.target.value })} placeholder="eg. Lot23" /></F>
              </div>
              <div className="rounded-lg border border-border bg-card p-4">
                <div className="mb-3 text-sm font-semibold text-primary">Driver Details</div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg border border-border bg-card p-4">
                  <div className="mb-3 text-sm font-semibold text-primary">Start Station</div>
                  <Select value={form.start_station} onValueChange={(v) => setForm({ ...form, start_station: v })}>
                ...
                <div className="rounded-lg border border-border bg-card p-4">
                  <div className="mb-3 text-sm font-semibold text-primary">End Station</div>
                  <Select value={form.end_station} onValueChange={(v) => setForm({ ...form, end_station: v })}>
                    <SelectTrigger><SelectValue placeholder="Choose station" /></SelectTrigger>
                    <SelectContent>{stations.map((s) => <SelectItem key={s.id} value={s.name}>{s.name}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
          ...
          <div className="mt-6">
            <div className="mb-2 text-sm font-semibold text-primary">Possible Shipment Consignments ({possibleConsignments.length})</div>
          ...
          <div className="mt-4">
            <div className="mb-2 text-sm font-semibold text-primary">Selected Shipment Consignments ({selected.length})</div>
            <div className="overflow-hidden rounded-lg border border-border">
              <table className="w-full text-sm">
          ...
          <DialogFooter className="mt-4"><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={save} className="bg-gradient-primary">Save</Button></DialogFooter>
        </DialogContent>
      </Dialog>
