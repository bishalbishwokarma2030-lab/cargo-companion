...
export const api = {
  stations: {
    ...
    update: async (id: string, s: Partial<Station>) => {
      const { data, error } = await supabase.from("stations").update(s).eq("id", id).select().single();
      if (error) throw error;
      return data as Station;
    },
  ...
  consignments: {
    ...
    update: async (id: string, c: Partial<Consignment>) => {
      const { data, error } = await supabase.from("consignments").update(c).eq("id", id).select().single();
      if (error) throw error;
      return data as Consignment;
    },
    remove: async (id: string) => {
